...
corporate_legend = {
    ...
    'xanchor': 'right',
    'x': 1.05,
	'font': {'size': 9, 'color': corporate_colors['light-grey']}} # Legend will be on the top right, above the graph, horizontally
...
@app.callback(
    Output('reporting-groups-l2dropdown-sales', 'options'),
    Input('reporting-groups-l1dropdown-sales', 'value'))
def l2dropdown_options(l1_dropdown_value):
    """

    :param l1_dropdown_value:
    :return:
    """
...
@app.callback(
	...
	[Input('date-picker-sales', 'start_date'),
     ...
     Input('reporting-groups-l1dropdown-sales', 'value'),
     Input('reporting-groups-l2dropdown-sales', 'value')])
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
    ...
    if isselect_all_l1 == 'N':
        """
        Hier wird zum ersten Mal über einen Label-Locator auf die Excel zugegriffen:
        """
    ...
    result = [df_1[sales_fields['sales']].sum()/1000000, df_1[sales_fields['revenues']].sum()/1000000, df_1[sales_fields['num_clients']].sum()/1000000]
    target = [df_1[sales_fields['sales_target']].sum()/1000000, df_1[sales_fields['rev_target']].sum()/1000000, '']
    performance = [df_1[sales_fields['sales']].sum()/df_1[sales_fields['sales_target']].sum(), df_1[sales_fields['revenues']].sum()/df_1[sales_fields['rev_target']].sum(), '']
...
@app.callback(
    Output('sales-count-day', 'figure'),
	[Input('date-picker-sales', 'start_date'), Input('date-picker-sales', 'end_date'),
     Input('reporting-groups-l1dropdown-sales', 'value'), Input('reporting-groups-l2dropdown-sales', 'value')])
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
    start = dt.strptime(start_date, '%Y-%m-%d')
    end = dt.strptime(end_date, '%Y-%m-%d')
    ...
    #Aggregate df
    val_cols = [sales_fields['sales'], sales_fields['sales_target']]
...
@app.callback(Output('sales-count-month', 'figure'), [Input('date-picker-sales', 'start_date'),
                                                      Input('date-picker-sales', 'end_date'),
                                                      Input('reporting-groups-l1dropdown-sales', 'value'),
                                                      Input('reporting-groups-l2dropdown-sales', 'value')])
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
    ...
    fig.add_trace(
        go.Scatter(
            x = df['month'],
            y = df[sales_fields['sales']]/df[sales_fields['sales_target']],
...
@app.callback(Output('sales-weekly-heatmap', 'figure'), [Input('date-picker-sales', 'start_date'),
                                                         Input('date-picker-sales', 'end_date'),
                                                         Input('reporting-groups-l1dropdown-sales', 'value'),
                                                         Input('reporting-groups-l2dropdown-sales', 'value')])
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
...
####################################################################################################
@app.callback(Output('sales-count-country', 'figure'), [Input('date-picker-sales', 'start_date'),
                                                        Input('date-picker-sales', 'end_date'),
                                                        Input('reporting-groups-l1dropdown-sales', 'value'),
                                                        Input('reporting-groups-l2dropdown-sales', 'value')])
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
...
####################################################################################################
@app.callback(Output('sales-bubble-county', 'figure'), [Input('date-picker-sales', 'start_date'),
                                                        Input('date-picker-sales', 'end_date'),
                                                        Input('reporting-groups-l1dropdown-sales', 'value'),
                                                        Input('reporting-groups-l2dropdown-sales', 'value')])
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
    ...
    #Aggregate df
    val_cols = [sales_fields['sales'], sales_fields['num_clients'], sales_fields['revenues']]
    df = df1.groupby(sales_fields['reporting_group_l1'])[val_cols].agg('sum')
    df.reset_index(inplace=True)
    df['rev_per_cl'] = df[sales_fields['revenues']]/df[sales_fields['num_clients']]
    del df1
    ...
    for co in country_names:
        ...
        fig.add_trace(
            go.Scatter(
                x=df_i['rev_per_cl'],
                y=df_i[sales_fields['num_clients']],
                name=co,
...
####################################################################################################
@app.callback(Output('sales-count-city', 'figure'), [Input('date-picker-sales', 'start_date'),
                                                     Input('date-picker-sales', 'end_date'),
                                                     Input('reporting-groups-l1dropdown-sales', 'value'),
                                                     Input('reporting-groups-l2dropdown-sales', 'value')])
def update_chart(start_date, end_date, reporting_l1_dropdown, reporting_l2_dropdown):
...
@app.callback(
    Output(component_id='adjclose_chart', component_property='figure'),
    Input('date-range', 'start_date'),
    Input('date-range', 'end_date'),
    Input('ticker', 'value'),    # value originates from layouts: options=[{"label": ticker, "value": ticker} for ticker in tickers

)
def page2_update_adjclose(ticker, start_date, end_date):
    """
    ...
    price_chart_figure = px.line(df_filtered_dates, x='Date', y=ticker, title='Time series with rangeslider and selectors')
