...
def fmp_earnings_surprises(reload_sp500=False):
    """
    ...
    for ticker in tqdm(tickers):
        ...
        earnings_surprises_url = 'https://financialmodelingprep.com/api/v3/earnings-surprises/' + ticker + '?apikey=' + key
        data_surprises = get_jsonparsed_data(earnings_surprises_url)

        print(data_surprises)
        #stopp = input('... hit Enter')

        # By default, the keys of the dict become the DataFrame columns:
        df = pd.DataFrame.from_dict(data_surprises)
        df['symbol'] = ticker
...
if __name__ == '__main__':
    main_df = fmp_earnings_surprises()
    print(main_df)
    print(type(main_df))
    push_df_to_db_replace(main_df, 'fmp_earnings_surprises')
    print(pull_df_wo_dates_from_db(sql='fmp_earnings_surprises'))
