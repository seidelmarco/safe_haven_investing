...
def fmp_quote(reload_sp500=False):
    """
    ...
    for ticker in tqdm(tickers):
        ...
        company_quote_url = 'https://financialmodelingprep.com/api/v3/quote/' + ticker + '?apikey=' + key
        data_quote = get_jsonparsed_data(company_quote_url)

        print(data_quote)
        #stopp = input('... hit Enter')

        # By default, the keys of the dict become the DataFrame columns:
        df = pd.DataFrame.from_dict(data_quote)
        #df['symbol'] = ticker
...
if __name__ == '__main__':
    ...
    print(main_df)
    print(type(main_df))
    push_df_to_db_replace(main_df, 'fmp_quote')
    # Todo: doesn't work due to parse dates in pull_df....
    # df = pull_df_from_db(sql='fmp_quote')