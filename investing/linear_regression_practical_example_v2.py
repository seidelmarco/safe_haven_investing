...
def linear_regression_practical_example_2():
    """
    ...
    # first improvement: we use 'Model' - what will happen?
    data_1 = raw_data
    #print(data_1)
    ...
    plt.savefig("statics/pictures/scatterplots_target_features_log_transformed.png", facecolor='white',
    ...
    print(data_with_dummies.head(10).tail(10))
    ...
    # Todo: here we include the 302 models - what will happen?
...
if __name__ == '__main__':
    linear_regression_practical_example_2()

"""
After including 308 unique models we suddenly explain 84% of variability (verrrry good) but we get absolute
unplausible and illogical coeffs/weights:

 Our model is explaining 84.07201385135853 % of the variability of the data
    
                      Features             Weights
0                      Mileage              -0.374
1                      EngineV               0.072
2                    Brand_BMW   -116435106150.995
3          Brand_Mercedes-Benz   -125461779829.219
4             Brand_Mitsubishi   1345506743691.578
5                Brand_Renault   8313846155587.072
6                 Brand_Toyota -13638616494676.412
7             Brand_Volkswagen   3228594201027.682
8                   Body_hatch              -0.064
9                   Body_other              -0.014
10                  Body_sedan              -0.059
11                  Body_vagon              -0.052
12                    Body_van              -0.058
13             Engine Type_Gas              -0.110
14           Engine Type_Other              -0.028
15          Engine Type_Petrol              -0.132
16            Registration_yes               0.300
17                   Model_100    -19674646727.274
18                    Model_11   -429442573943.433
19                   Model_116               0.031
20                   Model_118               0.043
21                   Model_120              -0.013
22                    Model_19   -607244960495.850
23                   Model_190               0.003
24                   Model_200               0.016
25                   Model_210               0.007
26                   Model_220               0.030
27                   Model_230               0.006
28                    Model_25   1906213172517.042
29                   Model_250               0.010
30                   Model_300              -0.006
31                   Model_316               0.021
32                   Model_318               0.051
...
36                   Model_328               0.030
37                   Model_330               0.059
38                   Model_335   2016477261102.121
39   Model_4 Series Gran Coupe               0.024
40                   Model_428               0.019
41               Model_4Runner    649601344872.682
42              Model_5 Series               0.020
43           Model_5 Series GT               0.050
44                   Model_520               0.159
45                   Model_523               0.047
46                   Model_524               0.001
47                   Model_525               0.074
...
52                   Model_545               0.021
53                   Model_550               0.049
54   Model_6 Series Gran Coupe               0.030
55                   Model_630               0.019
56                   Model_640               0.065
57                   Model_645               0.029
58                   Model_650               0.020
...
63                   Model_750               0.107
64                   Model_760               0.025
65                    Model_80    -12457867293.554
66                     Model_9  -3079533170800.034
67                    Model_90     -5089195085.477
68                 Model_A 140               0.014
69                 Model_A 150               0.019
70                 Model_A 170               0.020
71                 Model_A 180               0.006
72                    Model_A1     -8812464094.262
73                    Model_A3    -16857106455.422
74                    Model_A4    -37140560781.327
75            Model_A4 Allroad    -10174440210.119
76                    Model_A5    -19674646727.223
77                    Model_A6    -61699953876.838
78            Model_A6 Allroad    -23805520781.047
79                    Model_A7     -8812464094.229
80                    Model_A8    -27769924642.167
81                   Model_ASX   -209826515324.867
82                Model_Amarok   -226064155327.180
83                 Model_Auris   3425342966399.577

Getting very large coefficients from linear regression:

When I run the regression, the coefficients I am getting for each of the dummy variables and intercept is in the region
of 10E10 to 10E13.

Hint: drop those observations that have a scale far above the others. This way you will have a better understanding of 
the situation. It may happen that some high values (extreme values) are skewing the coefficients. 
Those coefficients seem too high (although it may be theoretically possible if positive coefficients are offset 
by negative coefficients of the same scale).. check also if some of those many variables are too strongly correlated, 
in which case there could be excessive multicollinearity (maybe you could have extremely high values for the 
correlations making the estimate unstable)

We have 302 distinct car models for around 4,000 observations. So we have a dummy var. e. g. Transporter T5 which 
produces in the regression formula only one True-value but more than 300 False-values. This could lead to extremely 
narrow behaviour

https://stats.stackexchange.com/questions/421508/getting-very-large-coefficients-from-linear-regression:

"You are right that something is wrong with this analysis. If a given predictor has 99,999 "0"s and a single "1," 
it is likely to produce an astronomical coefficient and standard error. Such distributions might be called 
"extremely narrow" or be said to feature "rare events." 
It sounds as if you have many predictors like this; they will be of no use in your regression. 
See also Large standard error for one predictor.

It is possible that the standard error in question is inflated because there are no observations where the predictor 
is different than zero when the response is equal to one"

Remedy: 
Either collect more data to allow the cells of your contingency table to fill in, 
or acknowledge that this predictor adds no predictive or explanatory information about this dependent variable.

In the future you'll want to check coefficient tables for any such "astronomical" values either of a coefficient 
or of its standard error.

It may be an interesting exercise to note the degree to which other results from the regression differ 
between versions that include and exclude the problematic predictor.
