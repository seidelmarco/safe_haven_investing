...
def fmp_growth(reload_sp500=False):
    """
    ...
    for ticker in tqdm(tickers):
        ...
        financial_growth_url = 'https://financialmodelingprep.com/api/v3/financial-growth/' + ticker + '?apikey=' + key
        data_fin_growth = get_jsonparsed_data(financial_growth_url)

        print(data_fin_growth)
        #stopp = input('... hit Enter')

        # By default, the keys of the dict become the DataFrame columns:
        df = pd.DataFrame.from_dict(data_fin_growth)
        #df['symbol'] = ticker
...
if __name__ == '__main__':
    ...
    print(main_df)
    print(type(main_df))
    push_df_to_db_replace(main_df, 'fmp_growth')

    df_fmp_growth = pull_df_wo_dates_from_db(sql='fmp_growth')
    print(df_fmp_growth)
